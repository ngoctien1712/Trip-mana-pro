...
import { delay, paginate, filterBySearch, sortBy, httpClient } from './http';
import {
  mockServices,
  mockOrders,
  mockOwnerDashboard,
...
export const ownerApi = {
  ...
  async listOrders(params: OrderListParams = {}): Promise<PaginatedResponse<Order>> {
    ...
    if (params.pageSize) searchParams.set('pageSize', (params.pageSize || 10).toString());

    const query = searchParams.toString();
    return httpClient.get<PaginatedResponse<Order>>(`/owner/orders${query ? `?${query}` : ''}`);
  },

  async getOrder(id: string): Promise<any> {
    const res = await httpClient.get<{ data: { order: any; details: any; payments: any[] } }>(`/owner/orders/${id}`);
    return {
      ...res.data.order,
